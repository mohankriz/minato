...
import Runner.auto.org.automation_runner;
...
public class multi_def extends Base_Proutil {
	...
	public void user_launch_the_website() {
		get_url("http://automationpractice.com/index.php?controller=authentication&back=my-account");
	}
	@When("user Enter The {string} In Username Flield")
	public void user_enter_the_in_username_flield(String string) throws InterruptedException {
		send_Keys(manager.getLogin().getMailid(), string);
		Thread.sleep(2000);

	}

	@When("user Enter The {string} In Passworld Flield")
	public void user_enter_the_in_passworld_flield(String string) throws InterruptedException {
		send_Keys(manager.getLogin().getPassword(), string);
		Thread.sleep(2000);
	}
